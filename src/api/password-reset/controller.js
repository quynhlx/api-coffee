...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Api Coffee account.<br>
        ...
        &mdash; Api Coffee Team
      `
      return sendMail({ toEmail: email, subject: 'Api Coffee - Password Reset', content })
    })
